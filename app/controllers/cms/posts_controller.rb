  ...
  class PostsController < ApplicationController
    ...
    def index
      if current_container
        ...
        @posts = current_container.container_posts.column_sort(params[:order], params[:direction]).paginate(:page => params[:page], :per_page => CMS::Post.per_page)
  
        @updated = @posts.blank? ? current_container.updated_at : @posts.first.updated_at
      else
        @title ||= 'Post'.t('Posts', 99)
        @posts = CMS::Post.column_sort(params[:order], params[:direction]).paginate(:page =>  params[:page])
        @updated = @posts.blank? ? CMS::Site.current.created_at : @posts.first.updated_at
      end

      @agents = CMS.agent_classes.map(&:all).flatten.sort{ |a, b| a.name <=> b.name }
      container_classes = CMS.container_classes - ( CMS.agent_classes + [ CMS::Site, CMS::Post ] )
